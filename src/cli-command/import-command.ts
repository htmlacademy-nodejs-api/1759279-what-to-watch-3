...
export default class ImportCommand implements CliCommandInterface {
  ...
  private async saveFilm(film: FilmType) {
    ...
    for (const {genreType} of film.genre) {
      const existGenre = await this.genreService.findByGenreNameOrCreate(genreType, {genreType});
      genre.push(existGenre.id);
    }
  ...
  public async execute(filename: string, login: string,
    password: string, host: string, dbname: string, salt: string): Promise<void> {
